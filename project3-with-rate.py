...
# Define token bucket parameters
TOKEN_BUCKET_CAPACITY = 10
TOKEN_BUCKET_REFILL_INTERVAL = timedelta(seconds=1)  # 1 second
...
# Token bucket refill function
def refill_token_bucket():
    global last_refill_time, token_bucket
    current_time = datetime.utcnow()
    time_since_last_refill = current_time - last_refill_time
    if time_since_last_refill >= TOKEN_BUCKET_REFILL_INTERVAL:
        tokens_to_add = int(time_since_last_refill.total_seconds() / TOKEN_BUCKET_REFILL_INTERVAL.total_seconds())
...
def jwt_ret():
    ...
    refill_token_bucket()  # Refill token bucket

    if token_bucket <= 0:
        print("Too many requests!")
        return "Too Many Requests", 429
    token_bucket -= 1  # Decrement token count
    token = encode_auth_token(exp, kid)  # Process request
